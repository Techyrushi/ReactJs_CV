...
const Wizard = React.memo(function Wizard(props) {
  ...
  const scale =
    size.width < 480 ? 0.048 : size.width < 768 ? 0.052 : size.width < 1024 ? 0.056 : 0.058;

  useFrame((state) => {
    if (!modelRef.current) return;
    modelRef.current.rotation.y += 0.007;
    modelRef.current.position.y = Math.sin(state.clock.elapsedTime) * 0.03;
  });
  ...
  return (
    <>
      <spotLight
        ...
        color="#fefe5b"
      />
      <Bounds fit clip={false} margin={1.7}>
        <group {...props} dispose={null} ref={modelRef}>
          <Center>
          <group scale={scale} rotation={[0.12, 0, 0]}>
      <mesh
        castShadow
        receiveShadow
